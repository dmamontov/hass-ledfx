...
async def test_devices(hass: HomeAssistant, httpx_mock: HTTPXMock) -> None:
    ...
    httpx_mock.add_response(
        text=load_fixture("devices_v2_data.json"), method=Method.GET
    )
    ...
    assert await client.devices() == json.loads(load_fixture("devices_v2_data.json"))
...
async def test_schema(hass: HomeAssistant, httpx_mock: HTTPXMock) -> None:
    ...
    httpx_mock.add_response(text=load_fixture("schema_v2_data.json"), method=Method.GET)

    client: LedFxClient = LedFxClient(
        get_async_client(hass, False), f"{MOCK_IP_ADDRESS}/", MOCK_PORT
    )

    assert await client.schema() == json.loads(load_fixture("schema_v2_data.json"))
...
async def test_config(hass: HomeAssistant, httpx_mock: HTTPXMock) -> None:
    ...
    httpx_mock.add_response(text=load_fixture("config_v2_data.json"), method=Method.GET)

    client: LedFxClient = LedFxClient(
        get_async_client(hass, False), f"{MOCK_IP_ADDRESS}/", MOCK_PORT
    )

    assert await client.config() == json.loads(load_fixture("config_v2_data.json"))
...
@pytest.mark.asyncio
async def test_colors(hass: HomeAssistant, httpx_mock: HTTPXMock) -> None:
    """colors test"""

    httpx_mock.add_response(text=load_fixture("colors_data.json"), method=Method.GET)

    client: LedFxClient = LedFxClient(
        get_async_client(hass, False), f"{MOCK_IP_ADDRESS}/", MOCK_PORT
    )

    assert await client.colors() == json.loads(load_fixture("colors_data.json"))

    request: Request | None = httpx_mock.get_request(method=Method.GET)
    assert request is not None
    assert request.url == get_url("colors")
    assert request.method == Method.GET


@pytest.mark.asyncio
async def test_virtuals(hass: HomeAssistant, httpx_mock: HTTPXMock) -> None:
    """virtuals test"""

    httpx_mock.add_response(text=load_fixture("virtuals_data.json"), method=Method.GET)

    client: LedFxClient = LedFxClient(
        get_async_client(hass, False), f"{MOCK_IP_ADDRESS}/", MOCK_PORT
    )

    assert await client.virtuals() == json.loads(load_fixture("virtuals_data.json"))

    request: Request | None = httpx_mock.get_request(method=Method.GET)
    assert request is not None
    assert request.url == get_url("virtuals")
    assert request.method == Method.GET
...
async def test_device_on(hass: HomeAssistant, httpx_mock: HTTPXMock) -> None:
    ...
    assert await client.device_on(
        MOCK_DEVICE, "wavelength(Reactive)", True
    ) == json.loads(load_fixture("device_on_data.json"))

    request: Request | None = httpx_mock.get_request(method=Method.POST)
    assert request is not None
    assert request.url == get_url(f"virtuals/{MOCK_DEVICE}/effects")
    assert (
        request.content
...
async def test_device_off(hass: HomeAssistant, httpx_mock: HTTPXMock) -> None:
    ...
    assert await client.device_off(MOCK_DEVICE, True) == json.loads(
...
async def test_preset(hass: HomeAssistant, httpx_mock: HTTPXMock) -> None:
    ...
    assert await client.preset(
        MOCK_DEVICE, "default_presets", "wavelength(Reactive)", "sunset-sweep", True
...
async def test_set_audio_device(hass: HomeAssistant, httpx_mock: HTTPXMock) -> None:
    ...
    assert await client.set_audio_device(0, True) == json.loads(
    ...
    assert request is not None
    assert request.url == get_url("config")
    assert request.content == b'{"audio": {"audio_device": 0}}'
    assert request.method == Method.PUT
