...
@pytest.mark.asyncio
async def test_update_device_sensors(hass: HomeAssistant) -> None:
    """Test update device sensors.
    ...
    with patch("custom_components.ledfx.updater.LedFxClient") as mock_client:
        ...
        unique_id = _generate_id("host_api", updater.ip)
        entry = registry.async_get(unique_id)
        ...
        state = hass.states.get(_generate_id("host_api", updater.ip))
        assert state.state == str(0)
        assert state.name == "host_api".replace("_", " ").title()
        assert state.attributes["icon"] == "mdi:api"
        assert state.attributes["attribution"] == ATTRIBUTION
